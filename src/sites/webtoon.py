...
class WebtoonDownloader(ComicSiteBase):
    ...
    def get_manga_metadata(self, url: str):
        ...
        try:
            ...
            # Cover image - Look for main series image, not chapter thumbnails
            ...
            # Author/Creator - Look for "Created by" pattern
    ...
    def get_chapter_links(self, url: str, use_cache: bool = True, db_manager=None, manga_id: Optional[int] = None) -> List[Tuple[str, str, str]]:
        ...
        try:
            ...
            print(f"Fetching chapter list from: {url} (full scan)")
            ...
            while page <= max_pages:
                ...
                if not chapter_list:
                    if page == 1:
                        print("Debug: Chapter list not found on first page")
                ...
                if not page_chapters:
                    print(f"No chapters found on page {page}, stopping pagination")
                    break
                    
                all_chapters.extend(page_chapters)
                print(f"Found {len(page_chapters)} chapters on page {page} (total: {len(all_chapters)})")
            ...
            print(f"Found {len(unique_chapters)} total unique chapters across {page - 1} pages")
    ...
    def _get_cached_chapters(self, db_manager, manga_id: int) -> List[Tuple[str, str, str]]:
        ...
        try:
            ...
            print(f"Error getting cached chapters: {e}")
            return []
    ...
    def _incremental_chapter_update(self, url: str, db_manager, manga_id: int, cached_chapters: List[Tuple[str, str, str]]) -> List[Tuple[str, str, str]]:
        ...
        try:
            if not cached_chapters:
                print("No cached chapters, performing full scan")
            ...
            print(f"Incremental update: Latest cached episode is {latest_cached_episode}")
            ...
            while page <= max_pages_to_check and not found_cached_episode:
                ...
                print(f"Checking page {page} for new chapters...")
                ...
                if not chapter_list:
                    print(f"No chapters found on page {page}, ending scan")
                    break
                ...
                for item in chapter_list:
                    try:
                        ...
                        if episode_number > latest_cached_episode:
                            ...
                            print(f"Found new chapter: Episode {episode_number}")
                        
                        elif episode_number <= latest_cached_episode:
                            found_cached_episode = True
                            print(f"Found cached episode {episode_number}, stopping scan")
                            break
                ...
                if not page_has_new_chapters and page > 1:
                    print(f"No new chapters on page {page}, scan complete")
                    break
                
                page += 1
            
            if page > max_pages_to_check:
                print(f"Reached page limit ({max_pages_to_check}), stopping scan")
            
            # Combine new chapters with cached chapters
            ...
            print(f"Incremental update complete: {len(new_chapters)} new chapters found, {len(all_chapters)} total")
            
            # Update database with new chapters
            if new_chapters:
                try:
                    ...
                    print(f"Added {len(new_chapters)} new chapters to database")
                except Exception as e:
                    print(f"Error updating database with new chapters: {e}")
    ...
    def _update_scan_tracking(self, db_manager, manga_id: int, chapters: List[Tuple[str, str, str]], total_pages: int):
        ...
        try:
            ...
            # Update manga table with scan tracking info
            import sqlite3
            ...
            print(f"Updated scan tracking: latest_episode={latest_episode}, total_pages={total_pages}, chapters_cached={len(chapters)}")
            
        except Exception as e:
            print(f"Error updating scan tracking: {e}")
